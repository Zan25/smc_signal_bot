...
Supported commands:
  ...
  /porto          — paper trading portfolio: balance, open positions, WR breakdown
"""
...
class TelegramCommandHandler:
    ...
    def _send_porto(self, chat_id: int) -> None:
        ...
        try:
            ...
            # ── WR breakdown (all time) ────────────────────────────────────────
            lines.append("")
            lines.append("━━ WIN RATE (ALL TIME) ━━")
            strategy_order = ["swing", "intraday", "scalp"]
            emoji_map = {"swing": "📈", "intraday": "⏱️", "scalp": "⚡"}
            stats = self._paper_trader.get_strategy_stats()
            if not stats:
                lines.append("_Belum ada closed trade._")
            else:
                for strat in strategy_order:
                    ...
                    exit_parts = []
                    if s["tp2"]:   exit_parts.append(f"TP2×{s['tp2']}")
                    if s["tp1"]:   exit_parts.append(f"TP1×{s['tp1']}")
                    if s["sl"]:    exit_parts.append(f"SL×{s['sl']}")
                    if s["sl_be"]: exit_parts.append(f"BE×{s['sl_be']}")
                    if s["expired"]: exit_parts.append(f"EXP×{s['expired']}")
                    if s["eod"]:   exit_parts.append(f"EOD×{s['eod']}")
                    exit_str = " | ".join(exit_parts) if exit_parts else "–"
                    lines += [
                        f"{emoji} *{strat.upper()}* {wr_emoji} {wr:.0f}% "
                        f"({s['wins']}W/{s['losses']}L, {s['total']} trade) "
                        f"{pnl_sign}${s['pnl']:.2f}",
                        f"   {exit_str}",
                    ]
